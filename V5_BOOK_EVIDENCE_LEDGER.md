...
| L-W1 | Lower PDF p.135/227 (糊涂118), corroborated structurally by pp.148–149/227 (乔帮主) | In bear-market context, the primary passage describes a leader's first post-top left-side pullback after a prior material upward wave: roughly 3–7 trading sessions and roughly 20%–25% decline. A separate trader independently links low-buy methods to bear-market conditions. | A for `3–7` and `20%–25%`; B/C for regime/leader/top/execution | **W01 = `DEFER_BEAR_PULLBACK_PREREGISTRATION`.** The literal numeric structure is frozen, but bear-market state, leader identity, prior-wave qualification, top anchor, drawdown basis, market/stock resonance, entry, exit and economic control are not machine-ready. No W01 return screen is authorized. |
...
For representation programs such as L01, structural validation does not inherit economic validity. Economic testing starts a new preregistered experiment. Source-readiness programs such as U03/R02/T02/W01 may stop before preregistration when the source is too ambiguous; that defer result cannot be bypassed by historical threshold search. Architecture programs such as R01 may freeze safe state/authorization behavior but cannot manufacture an `OPPORTUNITY_PRESENT` classifier or a sleeve authorization from prose/self-declaration.

## Next research queue

Priority is based on source clarity, representation quality and pre-result sample coverage, not on observed returns:

1. **Weak-market / bear-state representation evidence (L-W1 + L-R1):** W01 has a concrete pullback window and magnitude but cannot proceed until the market state is independently and causally represented. Seek stronger source or a non-P&L representation validation; do not search index/breadth thresholds against W01 returns.
2. **Leader/top representation evidence for W01:** reuse existing descriptive representations only if the semantic mapping is justified before returns; do not choose market-total/sector/return-rank leader proxies by whichever one profits most.
