...
1. **Opportunity / no-opportunity observables (L-R1):** continue source extraction for concrete, machine-defensible descriptions of when the market offers opportunity versus when cash/waiting is appropriate. Until a separate classifier is preregistered and validated, R01 remains `UNKNOWN -> CASH_ONLY`; do not invent breadth/index/sentiment cutoffs.
2. **Replacement-leader / follower source extraction (U-L3):** U03 is frozen deferred. Reopen only if a cleaner page/image or materially clearer passage resolves target identity, relationship, ranking and timing.
3. **Leader-stage economics (L-L1):** L02 is frozen rejected. Do not perform a stage-pair search. Another stage-economic test is allowed only if a distinct source passage gives a separately motivated economic hypothesis that can be preregistered before its returns are inspected.
