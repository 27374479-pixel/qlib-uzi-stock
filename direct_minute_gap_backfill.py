...
Design goals:
- single-threaded and rate-limited;
- existing complete local files are skipped;
- each successful symbol is persisted immediately;
- provider health circuit-breaker stops after consecutive failures;
...
def _covers(frame: pd.DataFrame, start: pd.Timestamp, end: pd.Timestamp) -> bool:
    ...
    # Start/end are date-level requirements. Allow one calendar week at each edge
    # because the requested boundary may be a holiday/weekend.
...
def main() -> None:
    ...
    manifest = {
        "start": str(start), "end": str(end), "sleep_seconds": args.sleep,
